...
int jail(int&,int[],int&,int&,int[],int&,bool&);
int chest (int&,int[],int&,int&,int&,int[],int&,int&,bool&,bool&,bool&);
int chance(int&,int[],int&,int&,int&,int[],int&,int&,bool&,bool&,bool&);
int pay(int &,int,int &,int,bool);

/*int chance (Human h, &bool)
 * {
 * 
 * }*/
...
//    int p1loc=0;//player location on the board ==== is now loc
    int c1=0; //computer dice 
    int c1bal=1500;//computer bank account
    int c1loc=0;//computer location on the board
                ...
                do{
                    if(game!=false){
                        ...
                        for(int i=1;i<=turn;i++){
                            ...
                            move(h.getloc(),h.getroll(),h.getMoney(),pmove);
                            cout<<"You landed on "<<property[h.getloc()]<<endl;
                            if(h.getloc()==4 || h.getloc()==38){
                                pay(h.getMoney(),h.getloc(),c1bal,c1loc,who);
                            }
                            if(h.getloc()==7 || h.getloc()==22 || h.getloc()==36){
                                chance(h.getloc(),pmove,h.getroll(),h.getMoney(),c1loc,cmove,c1,c1bal,prison,cprison,who);
                            }
                            if(h.getloc()==2 || h.getloc()==17 || h.getloc()==33){
                                chest(h.getloc(),pmove,h.getroll(),h.getMoney(),c1loc,cmove,c1,c1bal,prison,cprison,who);
                            }
                            if(h.getloc()==30){
                                prison=false;
                                jail(h.getloc(),pmove,h.getroll(),c1loc,cmove,c1,who);
                            }
                    ...
                    if(game!=false){
                        if(prison==false){
                            ...
                            for(int i=1;i<=turn;i++){
                                who=false;
                                c1=roll(6,2);
                                cout<<"Now it is the computers turn, balance $"<<c1bal<<"\n";
                                cout<<"The computer moved forward "<<c1<<" spaces."<<endl;
                                cmove[c1loc]=0;
                                c1loc+=c1;
                                compmove(c1loc,c1,c1bal,cmove);

                                cout<<"The computer landed on "<<property[c1loc]<<endl;
                                win(h.getMoney(),c1bal,game);//determines the winner
                                ...
                                for(int i=0;i<p1prop.size();i++){
                                    if(c1loc==p1prop[i]){
                                        c1bal-=rent[c1loc];
                                        h.setmoney(h.getMoney+rent[c1loc]);
                                        cout<<"You own "<<property[c1loc]<<" the computer owes you $"<<rent[c1loc]<<" for rent\n";
                                    }
                                }
                                int Cbuy=rand()%3+1;
                                if(avail[c1loc]==true){
                                   if(c1bal>buy[c1loc]){
                                       if(Cbuy==2){
                                          cout<<"The computer purchased "<<property[c1loc]<<endl;
                                          c1bal-=buy[c1loc];//Subrtracts the cost to buy from the players balance
                                          avail[c1loc]=false;//Is no longer available
                                          c1prop.push_back(c1loc);//stores the location of where the player owns 
                                        }
                                    }
                                }
                                display(pmove,cmove);
                                cout<<"Computer's turn ended, balance $"<<c1bal<<"\n";
                                cout<<"\n\n";
                            }
...
//int pay(int &p1bal,int p1loc,int &c1bal,int c1loc,bool who){
//    if(who==true){
//        if(p1loc==4){
...
//    }
//}
//int chance(int &p1loc,int pmove[],int &p1,int &p1bal,int &c1loc,int cmove[],int &c1,int &c1bal,bool &prison,bool &cprison,bool &who){
...
//                prison=false;
//                jail(p1loc,pmove,p1,c1loc,cmove,c1,who);
//                break;
//            }
//            case 2:{
...
//    }
//}
//int chest(int &p1loc,int pmove[],int &p1,int &p1bal,int &c1loc,int cmove[],int &c1,int &c1bal,bool &prison,bool &cprison,bool &who){
...
//    }
//}
//int jail(int &p1loc,int pmove[],int &p1,int &c1loc,int cmove[],int &c1,bool &who){
//    if(who==true){
//        pmove[p1loc]=0;
//        p1loc=10;
//        p1=0;
